<template>
  <b-container class="mb-5">
    ...
    <v-tabs-items v-model="tab">
      ...
      >
        <v-list-item>
          ...
          <v-card-actions>
            <b-btn
              text
              v-on:click.stop="buyDialog = true; buyToken = token; price = buyToken.price" variant="primary"
            >Buy</b-btn>
          </v-card-actions>
        </v-list-item>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="green darken-1" text @click="buyDialog = false; buyCard(buyToken.id,buyToken.price)">OK</v-btn>
      </v-card-actions>
      </v-card>
    </v-dialog>
      ...
      <b-col class="action-container">
        ...
        <b-form>
          <b-form-group id="matron" label="Matron ID:" label-for="matron">
            <b-form-input id="matron" v-model="matron" required placeholder="Enter Matron ID"></b-form-input>
          </b-form-group>
          ...
          <b-button v-on:click="breedKid()" type="button" variant="success">Breed Your Pokemon</b-button>
      ...
      >
        <v-list-item>
          ...
          <v-card-actions>
            ...
            <b-btn
              :disabled="token.onMarket" text
              v-on:click.stop="giveDialog = true; giveToken = token" variant="warning"
            >Give</b-btn>

            <b-btn :disabled="token.onMarket" text v-on:click="fightDialog= true; fightToken= token" variant="danger">Fight</b-btn>
    ...
    <v-dialog v-model ="fightResultDig" max-width="300" background-color="white">
      ...
      <v-card-text v-if ="fightResultDig" >You earned {{HP}} HP for your {{names[fightToken.genes-1]}}!</v-card-text>
      <v-img v-if ="fightToken!=null" v-bind:src="require('./assets/sprites/large/'+fightToken.url + '.png')"></v-img>  
      <v-card-actions>
        <v-spacer></v-spacer>
...
const contractAddress = "0x73e3b3Ff64780fB4A18aCC8c139f4A0a4C1Fc666";
...
export default {
  ...
  methods: {
    ...
    getTotalSupply(){
      this.crt.methods
        .totalSupply().call({ from: this.account }).then(res=>{
    ...
    fight(atk_id,def_id)
    {
      ...
      {
        this.fightAlert = true;
        this.alertmessage = "Oh man, you seemed choose a Pokemon that doesn't exist";
      }
      else{
          this.crt.methods.fight(atk_id,def_id).send({ from: this.account }).then(res=>{  
    ...
    give(giveToken,address){
        this.crt.methods.give(address,giveToken.id).send({ from: this.account })
        .then(receipt => {          
    ...
    breedKid() {
      ...
      for (var i = 0; i < this.myCards.length; i++) {
        if (this.matron == this.myCards[i].id) {
          flag1 = true;
        }
        if (this.sire == this.myCards[i].id) {
          flag2 = true;
        }
      }
      if (flag1 && flag2) {
    ...
    async getMarketCard(){
      ...
      const receipt = await this.crt.methods.getMarketCards().call({ from: this.account });
      ...
      for (let i = 0; i < receipt.length; i += 1) {
        ...
        let card_price = await this.crt.methods
          ...
          .call({ from: this.account });
        this.market_cards.push({id: card[0],
          genes: card[1],
          matron: card[2],
          sire: card[3],
          HP: card[4],
  ...
  //       this.cardInfo.id = card[0];
        
  //       this.cardInfo.genes =  card[1];
  //       this.cardInfo.matron= card[2];
  //       this.cardInfo.sire= card[3];
  //       this.cardInfo.HP= card[4];
  //       this.cardInfo.ATK= card[5];
  //       this.cardInfo.url =  card[1];
